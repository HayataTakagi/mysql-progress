...
function get_status_record() {
  mysql --defaults-file=.my.cnf -e "SHOW GLOBAL STATUS LIKE 'Handler_write'" | grep Handler_write | awk '{print $2}'
}
...
do
  ...
  echo "INITIAL: $INITIAL / CURRENT: $CURRENT/ DIFF: $CURRENT"
  echo "[ $DIFF_FROM_INIT records / $TOTAL records ($PROGRESS_PERCENTAGE %)] [$FORMARTED_ES]"
  sleep 1
done
